 ...
 * @LastEditTime: 2024-08-19 16:31:08
 * @Description: 垃圾箱
-->
<template>
    <y9Table
...
<script lang="ts" setup>
    ...
    import { deleteForeverEmail, emailList, flagEmail, moveToEmail, readEmail, searchEmail } from '@/api/email/index';
    ...
    function openEmailDetails(row) {
        ...
        router.push({ path: '/emailDetailInfo', query: { folder: row.folder, uid: row.uid, flag: -4 } });
        row.read = true;
    }
    ...
    function completelyDelete(e) {
        ...
        ElMessageBox.confirm(`${t('是否删除选中的邮件')}?`, t('提示'), {
            ...
            .then(async () => {
                ...
                let result = await deleteForeverEmail(folder.value, ids.join(','));
                loading.value = false;
                if (result.success) {
                    ElNotification({
                        title: t('成功'), //result.success ? t('成功') : t('失败'),
