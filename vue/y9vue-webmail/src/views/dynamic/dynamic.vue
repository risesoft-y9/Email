...
<script lang="ts" setup>
    ...
    async function onCollect(row) {
        ...
        console.log(row);
        let result;
        result = await flagEmail(row.uid, props.folder, !row.flagged);
        //处理完成后
        if (result.success) {
    ...
    function flag(type) {
        ...
        ElMessageBox.confirm(
            ...
            .then(async () => {
                ...
                switch (type) {
                    case 'read':
                        result = await readEmail(ids.join(','), props.folder, true);
                        break;
                    case 'unread':
                        result = await readEmail(ids.join(','), props.folder, false);
                        break;
                    case 'star':
                        result = await flagEmail(ids.join(','), props.folder, true);
                        break;
                    case 'unstar':
                        result = await flagEmail(ids.join(','), props.folder, false);
                        break;
                }
    ...
    async function move(toFolder) {
        ...
        result = await moveToEmail(ids.join(','), props.folder, toFolder);
        console.log(result);
        getEmailList();
    }
    ...
    function completelyDelete(e) {
        ...
        ElMessageBox.confirm(`${t('是否删除选中的邮件')}?`, t('提示'), {
            ...
            .then(async () => {
                ...
                let result = await deleteEmail(props.folder, ids.join(','));
                loading.value = false;
                if (result.success) {
                    ElNotification({
                        title: t('成功'), //result.success ? t('成功') : t('失败'),
