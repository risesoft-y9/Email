...
电子邮件，面向区域政府机构（省、市、区（县）、乡、镇）、行业管理部门（财政、水务、国土等）、大型企业集团、中小型企业和NGO（协会、组织）提供可私有化部署的电子邮件服务。[系统在线体验----->>>>>](#在线体验)
...
用户不用点击进入具体邮件信息即可滑动预览邮件内容，根据分类快速查看收件箱、发件箱、草稿箱等中的邮件信息。同时用户可以管理自己常用联系人，便于快速发送或回复邮件任务。

### 私有化部署

私有化部署指的是用户拥有自己的服务器，购买有生云的私有化版本，通过有生软件工程师安装到用户的服务器。私有化部署能够极大满足用户对安全性的要求。

### 前后端分离架构

实现前后端代码的解耦，能够灵活应对各种复杂多变的前端需求，提升开发效率，增强代码可维护性。

### 权限控制界面

电子邮件与数字底座对接后，可通过数字底座控制用户电子邮件权限，按照用户不同岗位需求给予对应操作权限。
